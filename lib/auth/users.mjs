...
import { PERSIST, dbUsersEmpty, dbLoadUsers, dbUpsertUser } from "../db/index.mjs";
...
const users = (g.__LM_USERS__ ??= hydrateUsers());
g.__LM_USER_SEQ__ ??= computeUserSeq();

/** In persist mode: seed the DB once (only when empty), then load every row —
 *  so registrations from earlier runs come back. Otherwise: original seed-only
 *  in-memory behaviour, unchanged. */
function hydrateUsers() {
  if (!PERSIST) return new Map(SEED.map((u) => [u.id, u]));
  if (dbUsersEmpty()) for (const u of SEED) dbUpsertUser(u);
  return new Map(dbLoadUsers().map((u) => [u.id, u]));
}

/** Continue the u-#### sequence past whatever is already stored, never below the
 *  3001 floor the created-account space starts at. */
function computeUserSeq() {
  let max = 3000;
  for (const id of users.keys()) {
    const m = /^u-(\d+)$/.exec(id);
    if (m) max = Math.max(max, Number(m[1]));
  }
  return Math.max(3001, max + 1);
}
